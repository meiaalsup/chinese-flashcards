...
async function pollDictStatus() {
  try {
    ...
    const el = $('dict-status');
    const lbl = el.querySelector('.dict-label');
    if (dictReady) {
      el.className = 'dict-status ready';
    ...
    el.querySelector('.dict-label').textContent = 'Dictionary error';
  }
}
...
let studyDirection = 'zh-en'; // 'zh-en' or 'en-zh'
...
$('gen-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    const { created } = await api('POST', '/api/generate', { text, groupId: groupId ? +groupId : null });
...
function buildCardItem(card) {
  ...
  const chinese = el('div', 'card-item-chinese', card.chinese || '—');
  const pinyin  = el('div', 'card-item-pinyin',  card.pinyin  || '');
  const english = el('div', 'card-item-english', card.english || 'no translation yet');
  div.append(chinese, pinyin, english);
  ...
  if (card.stats && card.stats.total > 0) {
    ...
    const pill = el('span', `stat-pill ${pct >= 70 ? 'good' : pct < 40 ? 'bad' : ''}`,
...
function openEditCard(card) {
  ...
  const makeField = (label, id, val, placeholder) => {
    const f = el('div', 'modal-field');
    const lbl = el('label', 'label', label);
    lbl.htmlFor = id;
    const inp = document.createElement('input');
    inp.type = 'text'; inp.id = id; inp.value = val || ''; inp.placeholder = placeholder || '';
    f.append(lbl, inp);
    return f;
  };
  ...
  body.append(
    ...
    makeField('Pinyin',  'edit-pinyin',  card?.pinyin,  'nǐ hǎo  (auto-filled from Chinese)'),
    makeField('English', 'edit-english', card?.english, 'hello  (auto-filled from Chinese)'),
  );
  ...
  $('edit-chinese').addEventListener('blur', async e => {
    ...
    if (!val) return;
    try {
      // Create a temp card to get pinyin+english, then immediately delete it
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      if (card) {
        await api('PUT', `/api/cards/${card.id}`, { chinese, pinyin, english });
      } else {
        await api('POST', '/api/cards', { chinese, pinyin, english });
      }
      await loadAll();
...
function renderGroups() {
  const smart = $('smart-groups');
  const custom = $('custom-groups');
  smart.innerHTML = ''; custom.innerHTML = '';

  allGroups.filter(g => g.is_smart).forEach(g => smart.appendChild(buildGroupCard(g)));
  const customs = allGroups.filter(g => !g.is_smart);
  if (!customs.length) {
    custom.innerHTML = '<div class="empty-state">No custom groups yet.</div>';
  } else {
    customs.forEach(g => custom.appendChild(buildGroupCard(g)));
  }
}
...
$('new-group-btn').addEventListener('click', () => {
  ...
  const nameLbl = el('label', 'label', 'Group Name'); nameLbl.htmlFor = 'new-group-name';
  const nameInp = document.createElement('input');
  nameInp.type = 'text'; nameInp.id = 'new-group-name'; nameInp.placeholder = 'e.g. HSK Level 1';
  nameField.append(nameLbl, nameInp);

  const colorField = el('div', 'modal-field');
  const colorLbl = el('label', 'label', 'Color'); colorLbl.htmlFor = 'new-group-color';
  const colorPicker = document.createElement('input');
  colorPicker.type = 'color'; colorPicker.id = 'new-group-color'; colorPicker.value = '#6366f1';
...
async function openGroupDetail(group) {
  ...
  if (cards.length) {
    const list = el('div', '');
    list.style.cssText = 'display:flex;flex-direction:column;gap:8px;margin-top:12px;max-height:360px;overflow-y:auto';
    cards.forEach(card => {
      const row = el('div', '');
      row.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px 10px;background:var(--bg3);border-radius:8px';
      const left = el('div', '');
      left.innerHTML = `<span style="font-size:20px">${card.chinese || ''}</span> <span style="color:var(--accent-h);font-size:13px">${card.pinyin || ''}</span> <span style="color:var(--text-dim);font-size:13px">${card.english || ''}</span>`;
      ...
      if (!group.is_smart) {
        ...
        rem.addEventListener('click', async () => {
          ...
          const cur = parseInt(infoLine.textContent);
          infoLine.textContent = `${cur - 1} card${cur - 1 !== 1 ? 's' : ''} in this group`;
        });
        row.appendChild(rem);
      }
    ...
    body.appendChild(el('p', 'empty-state', 'No cards in this group yet.'));
  }

  const footer = el('div', 'modal-footer');

  if (!group.is_smart) {
    const addCardsBtn = el('button', 'btn btn-ghost', 'Add cards');
    addCardsBtn.addEventListener('click', () => openAddCardsToGroup(group));
    footer.appendChild(addCardsBtn);
  }

  const studyBtn = el('button', 'btn btn-primary', 'Study this group');
...
async function openAddCardsToGroup(group) {
  $('modal-title').textContent = `Add cards to "${group.name}"`;
  const body = $('modal-body');
  body.innerHTML = '';
  ...
  searchInp.type = 'search'; searchInp.placeholder = 'Filter cards…';
  ...
  function renderAddList(filter) {
    ...
    visible.forEach(card => {
      ...
      const cb = document.createElement('input'); cb.type = 'checkbox'; cb.checked = selected.has(card.id);
  ...
  const footer = el('div', 'modal-footer');
  el('button', 'btn btn-ghost', 'Back').addEventListener('click', () => openGroupDetail(group));
...
async function renderStudyPicker() {
  ...
  allGroups.forEach(g => {
    ...
    const name  = el('div', 'group-name', g.name);
    const count = el('div', 'group-count', `${g.count} card${g.count !== 1 ? 's' : ''}`);
    const studyBtn = el('button', 'btn btn-sm study-now-btn', 'Study now →');
    div.append(name, count, studyBtn);
...
function startStudySession(groupId, groupName, cards) {
  ...
  $('flashcard').parentElement.style.display = '';
  $('answer-btns').style.display = 'none';

  // Configure card faces for direction
  const isZhEn = studyDirection === 'zh-en';
  $('front-zh').style.display = isZhEn ? '' : 'none';
  $('front-en').style.display = isZhEn ? 'none' : '';
  $('back-zh').style.display  = isZhEn ? '' : 'none';
  $('back-en').style.display  = isZhEn ? 'none' : '';
...
$('speak-btn').addEventListener('click', e => {
  e.stopPropagation();
  speak($('card-chinese').textContent);
  flashSpeakBtn('speak-btn');
});
$('speak-btn-back').addEventListener('click', e => {
  e.stopPropagation();
  speak($('card-chinese-back').textContent);
  flashSpeakBtn('speak-btn-back');
});
$('speak-btn-back2').addEventListener('click', e => {
  e.stopPropagation();
  speak($('card-chinese-back2').textContent);
  flashSpeakBtn('speak-btn-back2');
});

function flashSpeakBtn(id) {
  const btn = $(id);
...
async function recordAnswer(correct) {
  ...
  studyIndex++;
  if (studyIndex >= studyQueue.length) endSession(); else showCard();
}
...
function endSession() {
  ...
  $('done-stats').innerHTML = `
    ...
    <div><div class="done-stat-label">Cards reviewed</div>
